...
echo "Releasing Wellinic Meet SDK ${SDK_VERSION}"
...
# Now build and publish the Wellinic Meet SDK and its dependencies
echo "Building and publishing the Wellinic Meet SDK"
pushd ${THIS_DIR}/../
./gradlew clean
...
if [[ $MVN_HTTP == 0 ]]; then
    pushd ${MVN_REPO_PATH}
    git add -A .
    git commit -m "Wellinic Meet SDK + dependencies: ${SDK_VERSION}"
    popd
fi
